	...
	if(env_page_ws_get_size(curenv)==curenv->page_WS_max_size)
{
		...
		int try1=1;
		int try2=1;
	// get virtual address for victim
	while(stop)
	{
			if (try1)
		{
			...
			{
				uint32 victim_va=env_page_ws_get_virtual_address(curenv,curenv->page_last_WS_index);
			    uint32 perms1 = pt_get_page_permissions(curenv, victim_va);
			...
			if (try2)
			{
					...
					{
						uint32 victim_va=env_page_ws_get_virtual_address(curenv,curenv->page_last_WS_index);
					    uint32 perms2= pt_get_page_permissions(curenv, victim_va);
						if((perms2&PERM_USED)==0)
						{
						stop=0;
						try1=0;
						try2=0;
	...
	uint32 * Ptr_pa=NULL,victim_va=env_page_ws_get_virtual_address(curenv,curenv->page_last_WS_index);
	struct Frame_Info * VictimFrame = get_frame_info(curenv->env_page_directory,(void*)victim_va,&Ptr_pa);
	VictimFrame->isBuffered=1;
	VictimFrame->environment=curenv;
	VictimFrame->va=curenv->ptr_pageWorkingSet[curenv->page_last_WS_index].virtual_address;
	pt_set_page_permissions(VictimFrame->environment,VictimFrame->va,PERM_BUFFERED,PERM_PRESENT);
	uint32 bper=pt_get_page_permissions(curenv,VictimFrame->va );
	if ((bper&PERM_MODIFIED)==0)
	{
		bufferList_add_page(&free_frame_list, VictimFrame);
	}
	else
	{
		bufferList_add_page(&modified_frame_list, VictimFrame);
	...
	uint32 bufferperm=pt_get_page_permissions(curenv, fault_va);
	if((bufferperm & PERM_BUFFERED)!= 0)
	{
		pt_set_page_permissions(curenv, fault_va, PERM_PRESENT, PERM_BUFFERED);
		uint32 * page_ptr=NULL;
		struct Frame_Info *re ;
		re=	get_frame_info(curenv->env_page_directory,(void*)fault_va,&page_ptr);
		re->isBuffered=0;
		if((bufferperm & PERM_MODIFIED)!=0)
		{
			bufferlist_remove_page(&modified_frame_list, re);
		}
		else
		{
			bufferlist_remove_page(&free_frame_list, re);
		}
	}
	else
	{
		struct Frame_Info *fault =NULL;
		allocate_frame(&fault);
		map_frame(curenv->env_page_directory,fault,(void*)fault_va,PERM_USER|PERM_WRITEABLE);
...
if(env_page_ws_is_entry_empty(curenv,curenv->page_last_WS_index)==1)
  {
	uint32 rfva=ROUNDDOWN(fault_va,PAGE_SIZE);
	env_page_ws_set_entry(curenv,curenv->page_last_WS_index,rfva);
