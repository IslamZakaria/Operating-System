...
int createSemaphore(int32 ownerEnvID, char* semaphoreName, uint32 initialValue)
{
	...
	if (get_semaphore_object_ID(ownerEnvID,semaphoreName)==E_SEMAPHORE_NOT_EXISTS)
	{
		...
		int y = allocate_semaphore_object(&newsem);
		if (y==E_NO_SEMAPHORE)
		{
			return E_NO_SEMAPHORE;
		}
		semaphores[y].empty=0;
...
void signalSemaphore(int ownerEnvID, char* semaphoreName)
{
	...
	int semindex = get_semaphore_object_ID(ownerEnvID,semaphoreName);
		...
		if (semaphores[semindex].value<=0)
		{
		struct Env *curen=dequeue(&semaphores[semindex].env_queue);
		sched_insert_ready(curen);
		//enqueue(env_ready_queues,curen);
		curen->env_status=ENV_READY;
		}
	// Steps:
	//	1) Get the Semaphore
