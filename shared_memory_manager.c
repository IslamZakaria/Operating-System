...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	...
	int r = get_share_object_ID(ownerID,shareName);
	if (r!=E_SHARED_MEM_NOT_EXISTS)
	{return E_SHARED_MEM_EXISTS;}

	struct Share * Mysharedobj = NULL;
	int rett=allocate_share_object(&Mysharedobj);
	if (rett == E_NO_SHARE)
	{
		return E_NO_SHARE ;
	}
	...
	{	uint32 va = (uint32 )virtual_address;
		...
		{
		struct Frame_Info * myf =NULL;
		allocate_frame(&myf);
		map_frame(myenv->env_page_directory,myf,(void *)va,PERM_USER|PERM_WRITEABLE);
		add_frame_to_storage(Mysharedobj->framesStorage,myf,i);
		va+=PAGE_SIZE;

		}
		Mysharedobj->size=size*PAGE_SIZE;
		Mysharedobj->ownerID=ownerID;
		Mysharedobj->isWritable=isWritable;
		strcpy(Mysharedobj->name,shareName);
		Mysharedobj->references=1;
		//add_frame_to_storage(Mysharedobj->framesStorage,myf,i);
		return  rett;
	}
	// Steps:
...
{//TODO: [PROJECT 2019 - MS2 - [6] Shared Variables: Get] getSharedObject() [Kernel Side]
	struct Env* myenv = curenv; //The calling environment
	int y = get_share_object_ID(ownerID,shareName);
	if (y==E_SHARED_MEM_NOT_EXISTS)
	{
		return E_SHARED_MEM_NOT_EXISTS;
	}
	uint32 *va =virtual_address;
	for (int i = 0 ; i<(shares[y].size/PAGE_SIZE);i++)
	{
		struct Frame_Info* myfr=NULL;
		myfr=get_frame_from_storage(shares[y].framesStorage,i);
		int g = 0;
		if(shares[y].isWritable==1)
		{
			g=PERM_WRITEABLE;
		}
		map_frame(myenv->env_page_directory,myfr,(void *)va,PERM_USER|g);
		va+=PAGE_SIZE;
	}
	shares[y].references++;
	return y;
