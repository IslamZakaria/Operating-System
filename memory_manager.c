...
{
	uint32 * virt = kmalloc(PAGE_SIZE);
	uint32 phpt = kheap_physical_address((uint32)virt);
	phpt/=PAGE_SIZE;
	phpt=phpt<<12;
	phpt |= PERM_PRESENT|PERM_WRITEABLE|PERM_USER;
	ptr_page_directory[PDX(virtual_address)]=phpt;
	uint32 * ptrable =NULL;
	get_page_table(ptr_page_directory,(void *)virtual_address,&ptrable);
	int i = 0;
	while (i<1024)
	{
		ptrable[i]=0;
...
void __freeMem_with_buffering(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	for (int i = 0 ; i< size ; i ++)
	{
		...
		uint32 perms = pt_get_page_permissions(e,first_va);
		if (perms&PERM_BUFFERED)
		{
			uint32 *PTR_PT = NULL;
			struct Frame_Info * myframe =NULL;
			myframe = get_frame_info(e->env_page_directory,(void *)first_va,&PTR_PT);
			...
			pt_clear_page_table_entry(e,first_va);
		}
		//************2222222222222************************

		uint32 *PTR_PTt = NULL;
		struct Frame_Info * myframe1 =NULL;
		myframe1 = get_frame_info(e->env_page_directory,(void *)first_va,&PTR_PTt);
		if(myframe1!=NULL)
		{
			env_page_ws_invalidate(e,first_va);

			pt_clear_page_table_entry(e,first_va);
			//unmap_frame(e->env_page_directory,(void *)first_va);

			pf_update_env_page(e,(void *)first_va,myframe1);
		...
		uint32 *PTR_p=NULL;
		get_page_table(e->env_page_directory,(void *)first_va,&PTR_p);
		if (PTR_p!=NULL)
		{
			int countr=0;
			...
			{
				kfree(PTR_p);
				pd_clear_page_dir_entry(e,first_va);
			}
		}
		//*************444444444444**********************
